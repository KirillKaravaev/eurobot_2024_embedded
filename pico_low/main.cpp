...
void subscriber_callback(const void *msgin) {
  /*
  // std_msgs__msg__Int32 - это структура из одного элемента data(см хедеры в
  файле std_msgs/msg/int32.h ), msg - указатель, и чтобы получить значение по
  указателю
  ...
  */
  ...
  // Kinematics::rpm rpm; // Объявляем название структуры rpm внутри класса
  // Kinematics. Операцией объявления области видимости :: мы как бы смотрим
  // изнутри класса int motor1_rpm = rpm.motor1; //и тем самым видим уже его
  // элементы, включая искомую структуру rpm. И уже по месту даем ей название
  // rpm, а дальше по этому названию
...
int main() {
  ...
  */
  ...
  rclc_publisher_init_default(
      &rpm_publisher, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, Quaternion), "rpm_topic");
