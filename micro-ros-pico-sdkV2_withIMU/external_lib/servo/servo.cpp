...
void servo(int number, int angle ){
    if(number == 1){
        ...
        pwm_set_gpio_level(PWM_PIN_5, (250+angle*5.55) );
        
    }

    else if(number == 6){
        pwm_set_gpio_level(PWM_PIN_6, (250+angle*5.55) );
...
void servo_init(){
    ...
    gpio_set_function(PWM_PIN_5, GPIO_FUNC_PWM);
    gpio_set_function(PWM_PIN_6, GPIO_FUNC_PWM);
    ...
    uint slice_num5 = pwm_gpio_to_slice_num(PWM_PIN_5);
    uint slice_num6 = pwm_gpio_to_slice_num(PWM_PIN_6);
    ...
    pwm_set_clkdiv(slice_num5, PWM_DIV);
    pwm_set_clkdiv(slice_num6, PWM_DIV);
    ...
    pwm_set_wrap(slice_num5, PWM_WRAP);
    pwm_set_wrap(slice_num6, PWM_WRAP);
    

    //Задаем начальное заполнение шим 50%
    pwm_set_gpio_level(PWM_PIN_1, 250);
    pwm_set_gpio_level(PWM_PIN_2, 250);
    pwm_set_gpio_level(PWM_PIN_3, 250);
    pwm_set_gpio_level(PWM_PIN_4, 250);
    pwm_set_gpio_level(PWM_PIN_5, 250);
    pwm_set_gpio_level(PWM_PIN_6, 250);
